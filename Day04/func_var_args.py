...
# 가변인수와 일반인수를 동시에 사용하실 때는
# 일반 인수를 반드시 키워드 인수방식으로 전달하셔야 합니다.

# result = calc_points(97,100,80,100,55,60,'김철수') (x) calc_points() missing 1 required keyword-only argument: 'name'
...
사용자에게 map()을 사용해서 여러개의 값을 하나의 input()으로 
입력받은 후 get_max()에게 전달해서 가장 큰 값을 리턴받으세요
입력받을 값은 5개로 하겠습니다.
'''
def get_max(*nums):
    max = 0
